...
class MainWindow(QMainWindow):
    ...
    def ui_create_widget_handle_common_file(self):
        btn_handle_common_file = self.ui_set_button(txt=const.TEXT_BTN_HANDLE_COMMON_FILE,
                                                    command=self.add_files_to_compare)
    ...
    def ui_add_file_to_filelist(self, file: str):
        layout = self.findChild(QVBoxLayout, 'filelist')
        widget = self.ui_create_widget_with_h_layout([
            self.ui_set_label(txt=file),
            self.ui_set_button(txt='X', command=self.remove_file)
        ])
    ...
    def add_files_to_compare(self):
        file = self.open_file(const.TEXT_DIALOG_TITLE_OPEN_FILE_FOR_COMPARE)
        if file:
            self.add_file_to_file_list(file)
            self.ui_add_file_to_filelist(file)
